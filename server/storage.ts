...
export class DatabaseStorage implements IStorage {
  ...
  async getMissingCheckins(organizationId: string, weekOf?: Date, teamId?: string): Promise<Array<{ 
    ...
    try {
      ...
      for (const user of activeUsers) {
        ...
        const [exemption] = await db
          ...
          .where(
            and(
              eq(checkinExemptions.organizationId, organizationId),
              eq(checkinExemptions.userId, user.userId),
              eq(checkinExemptions.weekOf, targetWeek)
            )
          )
          .limit(1);
        
        // Skip if user was exempted
      ...
      // Return grouped by week (for now just the target week)
      return [{
        weekOf: targetWeek,
        users: missingUsers
      }];
