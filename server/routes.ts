...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Temporary fix endpoint to correct session organization ID
  app.post("/api/auth/fix-session-org", requireAuth(), async (req, res) => {
    console.log("🔧 Fixing session organization ID");
    ...
    try {
      ...
      console.log(`🔍 Current user: ${req.currentUser.email} (ID: ${req.currentUser.id})`);
      console.log(`🔍 Current session organization ID: ${(req.session as any)?.organizationId}`);
      console.log(`🔍 Current req.orgId: ${req.orgId}`);
      console.log(`🔍 Current user organizationId: ${req.currentUser.organizationId}`);
      
      // For Patrick Accounting user, set the correct organization ID
      if (req.currentUser.email === 'mpatrick@patrickaccounting.com') {
        const correctOrgId = 'c70086e0-1307-48c9-825c-a01ef11cc602';  // The correct ID from database
        
        // Get organization details
        const organization = await storage.getOrganization(correctOrgId);
        if (!organization) {
          return res.status(404).json({ message: "Organization not found with correct ID" });
        }
        
        // Update the session with the correct organization ID
        await setSessionUser(req, req.currentUser.id, correctOrgId, organization.slug);
        
        console.log(`✅ Session updated with correct organization ID: ${correctOrgId}`);
        
        // Return the corrected user data
        const correctedUser = await storage.getUser(correctOrgId, req.currentUser.id);
        if (!correctedUser) {
          return res.status(404).json({ message: "User not found in organization" });
        }
        
        res.json({
          message: "Session organization ID fixed",
          user: sanitizeUser(correctedUser),
          correctOrganizationId: correctOrgId,
          organizationName: organization.name,
          oldOrgId: (req.session as any)?.organizationId
        });
      } else {
        // For other users, just verify their organization is correct
        const user = await storage.getUserGlobal(req.currentUser.id);
        if (!user) {
        ...
        // Update the session with the correct organization ID
        await setSessionUser(req, user.id, correctOrgId, organization.slug);
        
        console.log(`✅ Session updated with organization ID: ${correctOrgId}`);
        
        res.json({
          message: "Session organization ID verified and updated",
      ...
      console.error("Error fixing session organization ID:", error);
      res.status(500).json({ message: "Failed to fix session" });
    }
  });
