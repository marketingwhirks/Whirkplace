...
-- Note the ID from above (should be 'whirkplace-org-id' or existing ID)

-- Step 3: Create or update the Super Admin user
-- Replace 'whirkplace-org-id' with the actual ID from Step 2 if different
INSERT INTO users (
  id, 
  email, 
  name, 
...
VALUES (
  ...
  'Matthew Patrick',
  'mpatrickSA',
  'whirkplace-org-id',  -- Replace with actual org ID if different
  'admin',
  true,  -- CRITICAL: This enables Super Admin access
  true,
  true,
  NOW(),
  NOW(),
  'local',
  '$2b$10$ZKXHwYqHYpQ7KdZwzRwHauR0c5Wy0XhqEfYhF6vVxX6OkIXzM2uHa'  -- Hashed: SuperAdmin2025!
)
ON CONFLICT (email) DO UPDATE SET 
  organization_id = 'whirkplace-org-id',  -- Replace with actual org ID
  is_super_admin = true,  -- CRITICAL: This enables Super Admin access
...
-- This ensures the Category Management interface has data to display
